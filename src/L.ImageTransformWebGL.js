L.ImageTransformWebGL = L.ImageOverlay.extend({
   options: {
        antialias: true,
        depth: false,
    ...
    _initGL: function() {
        ...
        // canvas.width = this._nextHighestPowerOfTwo(image.naturalWidth);
        // canvas.height = this._nextHighestPowerOfTwo(image.naturalHeight);
        ...
        // this.canvas = canvas;
        this.gl = gl;
		this.getCanvas({points: [[0,0], [ww, 0], [ww, hh], [0, hh]]});
    },
...
console.log(attr);
        var image = this._imgNode,
            // xsc = 256 * p[0][0] / this.ww,
            // xsc = 256 / this.ww,
            // ysc = 256 / this.hh,
            xsc = this.w,
            ysc = this.h,
...
L.ImageTransformWebGL.addInitHook(function () {
    if (L.Mixin.ContextMenu) {
		L.ImageTransformWebGL.include(L.Mixin.ContextMenu);
	}
});
